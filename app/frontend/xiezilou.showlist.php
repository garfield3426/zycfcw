...
class ShowList extends Page{
    ...
    function filter(){
        ...
        if(is_numeric($this->input['dj']))          $filter['dj'] = $this->input['dj'];//单价
        if(is_numeric($this->input['djl']))         $filter['zjl'] = $this->input['djl'];//单价输入低区间
        if(is_numeric($this->input['djh']))         $filter['zjh'] = $this->input['djh'];//单价输入高区间
    ...
    function getSqlWhere(){
        $sqlWhere = " where ftype=1 and fytype=245 and i.state=1 ";
       // $sqlWhere .= isset($this->filter['kw_state'])  ? " and i.state = {$this->filter['kw_state']}" : " and i.state <> 2";
		...
		//$sqlWhere .= isset($this->filter['dj'])  ? " and i.dj  = '{$this->filter['dj']}'" : '';//写字楼类型
		//处理单价
		if(isset($this->filter['dj'])){
			switch ($this->filter['dj']){
				case 1:
				 $sqlWhere .= " and i.dj<=10000";
				  break;
				case 2:
				  $sqlWhere .= " and i.dj >=10000 and i.dj <= 20000";
				  break;
				 case 3:
				  $sqlWhere .= " and i.dj >=20000 and i.dj <= 30000";
				  break;
				 case 4:
				  $sqlWhere .= " and i.dj >=30000 and i.dj <= 50000";
				  break;
				 case 5:
				  $sqlWhere .= " and i.dj >=50000";
				  break;
				 default:
				  echo "";
			}
		...
		//处理单价输入区间查询
		if(isset($this->filter['djh'])){
			$djl = isset($this->filter['djl'])?$this->filter['djl']:'0';
			$sqlWhere .= " and i.dj >={$djl} and i.dj <=  '{$this->filter['djh']}'";
		}
