...
class ShowList extends Page{
    ...
    function getData(){
        ...
        while(!$rs->EOF){
			if( $rs->fields['zxcd'] ){
				$rs->fields['zxcd'] = Category::getTitle($rs->fields['zxcd']);
			}
			if($rs->fields['sn_images']){
				$rs->fields['sn_images']= explode(',',$rs->fields['sn_images'] );
			}
			if($rs->fields['kjylb_sp']){
				$rs->fields['kjylb_sp']= explode(',',$rs->fields['kjylb_sp'] );
			}
    ...
    function filter(){
        ...
        if(is_numeric($this->input['lx']))          $filter['lx'] = $this->input['lx'];//商铺类型
        if(is_numeric($this->input['hy']))          $filter['hy'] = $this->input['hy'];//商铺行业
    ...
    function getSqlWhere(){
        $sqlWhere = " where ftype=1 and fytype=246";
        ...
        $sqlWhere .= isset($this->filter['sq'])  ? " and i.sq = '{$this->filter['sq']}'" : '';//处理朝向
		...
		$sqlWhere .= isset($this->filter['hy'])  ? " and i.kjylb_sp like('%{$this->filter['hy']}%')" : '';//行业
		$sqlWhere .= isset($this->filter['lx'])  ? " and i.splx_sp  = '{$this->filter['lx']}'" : '';//写字楼类型
		//处理总价
		if(isset($this->filter['zj'])){
			switch ($this->filter['zj']){
				case 1:
				 $sqlWhere .= " and i.zj<=100";
				  break;
				case 2:
				  $sqlWhere .= " and i.zj >=100 and i.zj <= 150";
				  break;
				case 3:
				  $sqlWhere .= " and i.zj >=150 and i.zj <= 200";
				  break;
				 case 4:
				  $sqlWhere .= " and i.zj >=200 and i.zj <= 300";
				  break;
				 case 5:
				  $sqlWhere .= " and i.zj >=300 and i.zj <= 500";
				  break;
				 case 6:
				  $sqlWhere .= " and i.zj >=500 and i.zj <= 1000";
				  break;
				 case 7:
		...
		if(isset($this->filter['mj'])){
			switch ($this->filter['mj']){
				case 1:
				 $sqlWhere .= " and i.jzmj<=20";
				  break;
				case 2:
				  $sqlWhere .= " and i.jzmj >=20 and i.jzmj <= 50";
				  break;
				case 3:
				  $sqlWhere .= " and i.jzmj >=50 and i.jzmj <= 100";
				  break;
				 case 4:
				  $sqlWhere .= " and i.jzmj >=100 and i.jzmj <= 150";
				  break;
				 case 5:
				  $sqlWhere .= " and i.jzmj >=150 and i.jzmj <= 200";
				  break;
				 case 6:
				  $sqlWhere .= " and i.jzmj >=200 and i.jzmj <= 500";
				  break;
				 case 7:
				  $sqlWhere .= " and i.jzmj >=500 and i.jzmj <= 1000";
				  break;
				case 8:
				  $sqlWhere .= " and i.jzmj >=1000";
				  break;
				default:
				  echo "";
			}
