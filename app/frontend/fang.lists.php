...
class Lists extends Page{
    ...
    function getData(){
        ...
        while(!$rs->EOF){
			...
			if($rs->fields['fytype']==1){
				$rs->fields['editLink'] = Core::getUrl('edit','esfang',array('id'=>$rs->fields['id']),'',true);
			}else{
				$rs->fields['editLink'] = Core::getUrl('edit','sale',array('id'=>$rs->fields['id']),'',true);
			}
    ...
    function filter(){
        ...
        if(strlen($this->input['ftype']))    	$filter['ftype'] = $this->input['ftype'];
    ...
    function getSqlWhere(){
        $sqlWhere = " where m_id={$this->sess->get('memberid')} ";
        $sqlWhere .= isset($this->filter['kw_state'])  ? " and state = {$this->filter['kw_state']}" : " and state <> 2 ";
		$sqlWhere .= isset($this->filter['ftype'])  ? " and ftype = '{$this->filter['ftype']}'" : '';
		$sqlWhere .= isset($this->filter['fytype'])  ? " and fytype = '{$this->filter['fytype']}'" : '';
