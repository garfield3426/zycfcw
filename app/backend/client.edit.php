...
class Edit extends Page{
    ...
    function export($item=null, $emsg=null){
        ...
        $pvar = array(
            ...
            'title' => $this->lang->get('p_client_editTitle'),
            'formAct' => Core::getUrl(),
        );
    ...
    function validate($i){
        $e = array();
        if(!$i['name']){
            $e['name'] = $this->lang->get('e_client_nameIsEmpty');
        }
        if(!is_numeric($i['state'])){
            $e['state'] = $this->lang->get('e_client_stateIsEmpty');
        }
        
        if(!strlen($i['content'])){
            $e['content'] = $this->lang->get('e_client_contentIsEmpty');
        }
        //将错误信息的key转换为表单的name
        $emsg = array();
