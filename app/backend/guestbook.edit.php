...
class Edit extends Page{
    ...
    function export($item=null, $emsg=null){
        ...
        $pvar = array(
            ...
            'title' => $this->lang->get('p_guestbook_editTitle'),
            'formAct' => Core::getUrl(),
        );
    ...
    function validate($i){
        $e = array();
        if(!$i['title']){
            $e['title'] = $this->lang->get('e_guestbook_titleIsEmpty');
        }elseif($this->isExist($i['title'])){
            $e['title'] = $this->lang->get('e_guestbook_titleIsExist');
        }
        if(!is_numeric($i['state'])){
            $e['state'] = $this->lang->get('e_guestbook_stateIsEmpty');
        }
        if(!strlen($i['lang'])){
            $e['lang'] = $this->lang->get('e_guestbook_langIsEmpty');
        }
        if(!strlen($i['content'])){
            $e['content'] = $this->lang->get('e_guestbook_contentIsEmpty');
        }
        //将错误信息的key转换为表单的name
        $emsg = array();
