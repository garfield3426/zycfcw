...
class Edit extends Page{
    ...
    function process(){
        ...
        if(!is_numeric($this->currentId)){
            Core::raiseMsg($this->lang->get('e_systemUser_idIsEmpty'));
        }
        //检查是否提交数据
    ...
    function export($item=null, $emsg=null){
        ...
        $pvar = array(
            ...
            'title' => $this->lang->get('p_systemUser_editTitle'),
            'formAct' => Core::getUrl(),
            'passHint' => $this->lang->get('p_systemUser_userpassHint'),
        );
    ...
    function updateDb($item){
        ...
        $item['pid'] = is_numeric($item['pid']) ? $item['pid'] : 1;//获得pid
        if(!class_exists('Treenode')) include_once(LIB_DIR.'/treenode.class.php');
        $node = new Treenode($this->tab, $this->db);
        $node->updateNode($this->currentId, $item);
        $node->buildTree(1,1);
        $node->updateLevel(1);
        $node->buildTree(1,1);
        //清除缓存
        IO::removeFile(VAR_DIR.'/category');
    }
