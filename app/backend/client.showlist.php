...
class ShowList extends Page{
    ...
    function process(){
        $pvar = array(
            ...
            //页面标题
            'title' => $this->lang->get('p_client_showlistTitle'),
            //form Action
    ...
    function getData(){
       ...
       $type = unserialize(CLIENT_TYPE);
        while(!$rs->EOF){
            $rs->fields['put_time'] = date('Y-m-d',$rs->fields['put_time']);
            $rs->fields['types'] = $type[$rs->fields['types']];
            $rs->fields['reply_time'] = date('Y-m-d',$rs->fields['reply_time']);

            //处理性别
            $rs->fields['sex'] = $rs->fields['sex']==1 ? $this->lang->get('global_sexAsMan') : $this->lang->get('global_sexAsFemale');
            //处理地区
    ...
    function filter(){
        ...
        if(is_numeric($this->input['kw_types']))    $filter['kw_types'] = $this->input['kw_types'];
        ...
        if(strlen($this->input['kw_name']))        $filter['kw_name'] = $this->input['kw_name'];
    ...
    function getSqlWhere(){
        ...
        $sqlWhere .= isset($this->filter['kw_types'])  ? " and types = {$this->filter['kw_types']}" : '';
        ...
        $sqlWhere .= isset($this->filter['kw_reply'])  ? ($this->filter['kw_reply'] ? " and reply_time<>0" : " and reply_time=0") : '';
    ...
    function getJsonData(){
        return array(
            ...
            'kw_reply' => array(
                ...
                'list' => array(
                    '0' => $this->lang->get('p_client_NotReply'),
                    '1' => $this->lang->get('p_client_IsReply'),
                ),
            ),
            ...
            'mulop' => array(
                'list' => array(
                    'Enable' => $this->lang->get('p_client_mulopEnable'),
                    'Disable' => $this->lang->get('p_client_mulopDisable'),
                    'disabled_1' => '------',
                    'Delete' => $this->lang->get('p_client_mulopDelete'),
                ),
                'action' => array(
                    'type' => 'mulopChoose',
                    'Enable' => array(
                        'msg' => $this->lang->get('j_client_mulopEnable'),
                        'url' => Core::getUrl('state', '', array('value'=>1)),
                    ),
                    'Disable' => array(
                        'msg' => $this->lang->get('j_client_mulopDisable'),
                        'url' => Core::getUrl('state', '', array('value'=>0)),
                    ),
                    'Delete' => array(
                        'msg' => $this->lang->get('j_client_mulopDelete'),
            ...
            'kw_types' => array(
                'value' => $item['types'],
                'list' => $this->lang->get('client_type'),
            ),
            //操作链接事件
            'deleteLink' => array(
                'msg' => $this->lang->get('j_client_deleteItemMsg'), 
                'url' => Core::getUrl('state', '', array('value'=>2)),
            ),
            'editLink' => array(
