...
class ShowList extends Page{
    ...
    function process(){
        $pvar = array(
            ...
            //页面标题
            'title' => $this->lang->get('p_branch_showlistTitle'),
            //form Action
    ...
    function getData(){
        //sql查询语句
        $sqlQuery =
            "select i.id as id, i.state as state, i.name as name,i.is_order as is_order, u.username as editor
            from {$this->tab} i left join {$this->tab_user} u on i.editor=u.id";
        $sqlWhere = $this->getSqlWhere();
        $sqlOrder = " order by i.is_order asc";
        //保存查询数据到$list
        $list = array();
    ...
    function getSqlWhere(){
        $sqlWhere = " where h_id=".CONTENT_SKY;
    ...
    function getJsonData(){
        return array(
            ...
            'mulop' => array(
                'display' => true,
                'list' => array(
                    'Enable' => $this->lang->get('p_branch_mulopEnable'),
                    'Disable' => $this->lang->get('p_branch_mulopDisable'),
                    'disabled_1' => '------',
                    'Delete' => $this->lang->get('p_branch_mulopDelete'),
                ),
                'action' => array(
                    'type' => 'mulopChoose',
                    'Enable' => array(
                        'msg' => $this->lang->get('j_branch_mulopEnable'),
                        'url' => Core::getUrl('state', '', array('value'=>1)),
                    ),
                    'Disable' => array(
                        'msg' => $this->lang->get('j_branch_mulopDisable'),
                        'url' => Core::getUrl('state', '', array('value'=>0)),
                    ),
                    'Delete' => array(
                        'msg' => $this->lang->get('j_branch_mulopDelete'),
            ...
            //操作链接事件
            'deleteLink' => array(
                'msg' => $this->lang->get('j_branch_deleteItemMsg'), 
                'url' => Core::getUrl('state', '', array('value'=>2)),
            ),
            'editLink' => array(
