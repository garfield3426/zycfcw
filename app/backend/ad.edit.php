...
class Edit extends Page{
    ...
    function export($item=null, $emsg=null){
        ...
        $pvar = array(
            ...
            'title' => $this->lang->get('p_ad_editTitle'),
            'formAct' => Core::getUrl(),
        );
    ...
    function validate($i){
        $e = array();
        if(!$i['title']){
            $e['title'] = $this->lang->get('e_ad_titleIsEmpty');
        }elseif($this->isExist($i['title'])){
            $e['title'] = $this->lang->get('e_ad_titleIsExist');
        }
        if(!$i['logo']){
            $e['logo'] = $this->lang->get('e_ad_logoIsEmpty');
        }
        if(!$i['url']){
            $e['url'] = $this->lang->get('e_ad_urlIsEmpty');
        }
        //将错误信息的key转换为表单的name
        $emsg = array();
