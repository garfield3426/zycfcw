...
class ShowList extends Page{
    ...
    function getData(){
       ...
       $type = unserialize(BESPEAK_TYPE);
        while(!$rs->EOF){
            ...
            $rs->fields['operation_time'] = date('Y-m-d',$rs->fields['operation_time']);
            //处理性别
    ...
    function filter(){
        ...
        if(strlen($this->input['kw_cdkey']))        $filter['kw_cdkey'] = $this->input['kw_cdkey'];
    ...
    function getSqlWhere(){
        ...
        $sqlWhere .= isset($this->filter['kw_cdkey'])  ? " and cdkey = '{$this->filter['kw_cdkey']}'" :'';
        $sqlWhere .= isset($this->filter['kw_title'])  ? " and name like('%{$this->filter['kw_title']}%')" : '';
