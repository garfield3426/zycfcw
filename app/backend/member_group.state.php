...
class State extends Action{
    ...
    function process(){
        ...
        if(0 > $val || 2 < $val){
            Core::raiseMsg($this->lang->get('e_memberGroup_valueisimproper'));
        }
        if(is_array($id)){
            $idStr = implode(',',$id);
        ...
        case 2:
            ...
            $this->removePermissionsFile($id);
            Core::log(L_NOTICE, sprintf($this->lang->get('log_memberGroup_delete'), $idStr));
            break;
        }
    ...
    function removePermissionsFile($id){
        ...
        foreach($id as $v){
            IO::removeFile(VAR_DIR."/group_priv/".$this->lang->get('frontednId')."/{$v}");
        }
    }
}
?>
