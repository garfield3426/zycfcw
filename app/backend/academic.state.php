...
class State extends Action{
    ...
    function process(){
        ...
        if(0 > $val || 2 < $val){
            Core::raiseMsg($this->lang->get('e_video_ValueIsImproper'));
        }
        if(is_array($id)){
            $idStr = implode(',',$id);
        }elseif(!is_numeric($id)){
            Core::raiseMsg($this->lang->get('e_video_idIsEmpty'));
        }else{
            $idStr = $id;
        }
        if(2!=$val){
            ...
            $this->db->Execute($sql);
            Core::log(L_NOTICE, sprintf($this->lang->get('log_video_delete'), $idStr));
        }
