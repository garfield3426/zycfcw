...
class Edit extends Page{
    ...
    function process(){
        ...
        if(!is_numeric($this->currentId)){
            Core::raiseMsg($this->lang->get('e_adversite_idIsEmpty'));
        }
        //检查是否提交数据
    ...
    function export($item=null, $emsg=null){
        ...
        $pvar = array(
            ...
            'title' => $this->lang->get('p_adversite_editTitle'),
            'formAct' => Core::getUrl(),
        );
    ...
    function isExist($title){
        $sql = "select count(*) from {$this->tab} where sign='{$title}' and id<>{$this->currentId}";
    ...
    function validate($i){
        ...
        if(!$i['logo']){
            $e['logo'] = $this->lang->get('e_adversite_logoIsEmpty');
        }
        if(!$i['url']){
