...
class alipay_notify {
    ...
    function notify_verify(){   //对notify_url的认证
        ...
        $this->mysign = $this->sign($this->myprestr.$prestr.$this->security_code);
    ...
    function para_filter($parameter){ //除去数组中的空值和签名模式
        ...
        while(list($key, $val) = each($parameter)){
            if($key == "sign" || $key == "sign_type" || $key == "mod" || $key == "act" || $val == ""){
                continue;
            }else{
    ...
    function charset_encode($input,$_output_charset ,$_input_charset ="utf-8" ){
        ...
        if($_input_charset == $_output_charset || $input ==null ){
            ...
            return "sorry, you have no libs support for charset change.";
        }
        return $output;
    }
