...
jQuery.cookie = function(name, value, options) {
 ...
 (function($){$.extend({ImportBasePath:'',fileinfo:function(data){data=data.replace(/^\s|\s$/g,"");var m;if(/\.\w+$/.test(data)){m=data.match(/([^\/\\]+)\.(\w+)$/);if(m){if(m[2]=='js'){return{filename:m[1],ext:m[2],tag:'script'}}else if(m[2]=='css'){return{filename:m[1],ext:m[2],tag:'link'}}else{return{filename:m[1],ext:m[2],tag:null}}}else{return{filename:null,ext:null}}}else{m=data.match(/([^\/\\]+)$/);if(m){return{filename:m[1],ext:null,tag:null}}else{return{filename:null,ext:null,tag:null}}}},fileExist:function(filename,filetype,attrCheck){var elementsArray=document.getElementsByTagName(filetype);for(var i=0;i<elementsArray.length;i++){if(elementsArray[i].getAttribute(attrCheck)==$.ImportBasePath+filename){return true}}return false},createElement:function(filename,filetype){switch(filetype){case'script':if(!$.fileExist(filename,filetype,'src')){var scriptTag=document.createElement(filetype);scriptTag.setAttribute('language','javascript');scriptTag.setAttribute('type','text/javascript');scriptTag.setAttribute('src',$.ImportBasePath+filename);return scriptTag}else{return false}break;case'link':if(!$.fileExist(filename,filetype,'href')){var styleTag=document.createElement(filetype);styleTag.setAttribute('type','text/css');styleTag.setAttribute('rel','stylesheet');styleTag.setAttribute('href',$.ImportBasePath+filename);return styleTag}else{return false}break;default:return false;break}},cssReady:function(index,callback){function check(){if(document.styleSheets[index]){if(typeof callback=='function'){callback()}window.clearInterval(checkInterval)}}var checkInterval=window.setInterval(check,200)},include:function(file,callback){var headerTag=document.getElementsByTagName('head')[0];var fileArray=[];typeof file=='string'?fileArray[0]=file:fileArray=file;for(var i=0;i<fileArray.length;i++){var elementTag=$.fileinfo(fileArray[i]).tag;var el=[];if(elementTag!==null){el[i]=$.createElement(fileArray[i],elementTag);if(el[i]){headerTag.appendChild(el[i]);if($.browser.msie){el[i].onreadystatechange=function(){if(this.readyState==='loaded'||this.readyState==='complete'){if(typeof callback=='function'){callback()}}}}else{if(elementTag=='link'){$.cssReady(i,callback)}else{el[i].onload=function(){if(typeof callback=='function'){callback()}}}}}}else{return false}}}})})(jQuery);
	...
	function sizeWin(){
		...
		$("#content").css({'height': h+'px', 'width': w+'px'  }); 
	}
...
function T(el,tag){
	return G(el).getElementsByTagName(tag)
}
...
function Class(props,superClass){
	...
	if(superClass){
		function s_(){};
		s_.prototype=superClass.prototype;clazz.prototype=new s_();
	}
	for(var k in props){
...
Element.prototype={
	wrapVersion:1,css:function(arg){
		var argLen=arguments.length;
		if(argLen>1&&argLen%2==0){
			for(var i=0;i<argLen;i+=2){
				var a=arguments[i],a2=arguments[i+1];
				if(typeof a=='string'&&typeof a2=='string'){this.style[a]=a2;}
			}
		}else if(argLen==1){
			if(typeof arg=='string'){
				var sty=this.currentStyle||document.defaultView.getComputedStyle(this,null);return sty[arg];
			}else{
				for(var k in arg){this.style[k]=arg[k];}}
		}
		return this;
	},
	...
	removeClass:function(name){
		...
		while(len--){if(classArr[len]==name){classArr.splice(len,1);}}
...
function trim(str){str=''+str;return str.replace('/(^[\s\u3000\xa0]+|[\s\u3000\xa0]+$)/g','');};
...
Focus.prototype={
	init:function(){
		...
		for(var i=0;i<me.iTabsCount;i++){
			me.iTabs[i].index=i;
			G(me.iMods[i]).setOpacity(0);
			me.iTabs[i].onclick=me.modSwitch();
		}
		G(me.iMods[me.currentIndex]).setOpacity(1);
		G(me.iTabs[me.currentIndex]).className="current";
		me.autoPlay();
	},
	modSwitch:function(){
		var me=this;
		return function(e){
			e=e||window.event;
			var tar=e.srcElement||e.target;window.clearInterval(me.timer);
			if(G(tar).className!="current"){
				me.state=1;
				if(me.switchState==1){
					...
					me.nextIndex=tar.getAttribute('title');
				}
				me.nextIndex=tar.getAttribute('title');
				me.switchEffect();
			}else{
	...
	switchEffect:function(){
		...
		window.clearInterval(me.fadeInTimer);
		window.clearInterval(me.fadeOutTimer);
		G(me.iTabs[me.currentIndex]).className="";
		G(me.iTabs[me.nextIndex]).className="current";
		var timerEnd=1;
		var timerStart=0;
		me.fadeOutTimer=window.setInterval(
			
			function(){
				window.clearInterval(me.fadeInTimer);
				G(me.iMods[me.currentIndex]).setOpacity(timerEnd-0.1);
				timerEnd=timerEnd-me.step;
				if(timerEnd<0.8&&timerStart<1){
					G(me.iMods[me.nextIndex]).setOpacity(timerStart+0.1);
					timerStart=timerStart+me.step;
				}
				if(timerEnd<0){
					window.clearInterval(me.fadeOutTimer);
					G(me.iMods[me.currentIndex]).css("display","none");
					G(me.iMods[me.nextIndex]).css("display","");
					me.fadeInTimer=window.setInterval(
...
var Banner = {
	init:function(data){
		...
		for(var i=0, n=data.length; i<n; i++){
			...
			var id=data[i]["imgsrc"].replace(/.*\/(\d+)\.(jpg|jpeg|png|gif|bmp)$/i,"$1");
			
			html1.push('<a id="Banner'+id+'" href="'+data[i]["url"]+'" target="_blank"><div style="background:url('+data[i]["imgsrc"]+')"></div></a>');
			
			html2.push('<a href="javascript:void(0);" title="'+i+'">'+(i+1)+'</a>');
		} 
		G("BannerWrap").innerHTML=html1.join("");
		G("BannerIndex").innerHTML=html2.join("");
		if(n>1){
			var list=new Focus({iTabs:T("BannerIndex","a"),iMods:T("BannerWrap","div")});
		}
	} 
}; 
